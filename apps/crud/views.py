...
def check_student(student_name):
    ...
    student_day_records = StudentRecord.query.filter_by(student_id=student.id).filter(f"%{StudentRecord.started_at}%".like(f"%{date}%")).all()
    year = datetime.now().year
    student_records = StudentRecord.query.filter_by(student_id=student.id).order_by(StudentRecord.started_at.desc()).filter(f"%{StudentRecord.started_at}%".like(f"%{year}%")).all()
